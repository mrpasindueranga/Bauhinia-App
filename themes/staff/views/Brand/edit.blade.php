...
@section('content')
    <div class="leading-loose">
        <form action="{{ route('staff.brand.update', $Brand->id) }}" method="POST"
            class="max-w-xl m-4 p-10 rounded shadow-xl" enctype="multipart/form-data" style="background-color:#202125;">
            @csrf
            @method('put')
            ...
            <div class="flex items-center py-6">
                <div class="w-12 h-12 mr-4 flex-none rounded-xl border overflow-hidden">
                    <img class="w-12 h-12 mr-4 object-cover" src="{{ asset('storage\products\\' . $Brand->icon) }}"
                        alt="Avatar Upload">
                </div>
            ...
            <div>
                ...
                <input class="w-full px-5 focus:ring-green-500 py-1 text-gray-700 bg-gray-200 rounded" name="brand"
                    type="text" required placeholder="Brand Name" value="{{ $Brand->brand }}">
            </div>
            <div class="mt-2">
                ...
                <select required class="w-full px-5 focus:ring-green-500 py-1 text-gray-700 bg-gray-200 rounded"
                    ...
                    @foreach ($Measurements as $Measurement)
                        @if ($Brand->measurement == $Measurement->id)
                            <option value="{{ $Measurement->id }}" selected>{{ $Measurement->category }}</option>
                        @else
                            <option value="{{ $Measurement->id }}">{{ $Measurement->category }}</option>
                        @endif
                    @endforeach
                </select>
            </div>
            <div class="mt-2 w-full flex justify-between">
                <div style="width: 45%;">
                    ...
                    <input class="w-full px-5 focus:ring-green-500 py-1 text-gray-700 bg-gray-200 rounded" name="price"
                        type="number" required placeholder="Brand Price" value="{{ $Brand->price }}">
                </div>
                <div style="width: 45%;">
            ...
            <div class="mt-2">
                ...
                <textarea rows="5" required
                    ...
                    name="description" placeholder="Type Product Description">{{ $Brand->description }}</textarea>
            </div>
            <div class="mt-4">
